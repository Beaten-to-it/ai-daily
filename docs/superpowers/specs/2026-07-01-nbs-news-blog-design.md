...
- **동시실행 락 (R1 반영):** orchestrate 진입 즉시 `flock`(또는 pidfile) 락 획득 — 이미 실행 중이면 즉시 종료(exit=busy). ~15분 파이프라인 + 일일 timer(P3c) 겹침 시 git index 경합·이중 push 방지. 락은 orchestrate 소유(스케줄러가 P3c여도 여기서 건다).
- **날단위 멱등 3-상태 가드 (R1 반영 — published-but-push-failed 3번째 상태 처리):** `--force` 아니면 시작 시 판정:
  - **① 이미 배포됨** (`publish.json.status==published` & `pushed==true`) → no-op `status=skipped`, exit 0.
  - **② 로컬 발행됨·push 대기** (`publish.json.status==published` & `pushed!=true` & `git cat-file -e HEAD:content/news/<date>.md` 성공=그날 commit이 HEAD에 있음) → **스테이지 전부 스킵, push만 재시도**(재생성 안 함=결정적 복구). push 성공 시 published, 계속 실패면 `push_pending`. ← 근거: fail-fast 전체재실행 + 비결정적 `claude -p`이므로, push만 실패한 날을 전체 재실행하면 다른 에디션·발산 커밋이 됨. 이 분기가 그걸 막는다.
  - **③ 그 외** → 전체 파이프라인.
  - scratch(`runs/`)가 지워져 publish.json이 없어도 ②의 `cat-file HEAD:content/news/<date>.md`가 "로컬 발행됨"의 신뢰 신호(단, pushed 여부는 로컬 `origin/main` ref만으론 확증 불가 → 안전하게 push 재시도; push는 이미-반영이면 no-op). (`merge-base --is-ancestor`는 stale ref라 폐기.) `--force`가 유일한 강제 전체 재발행 경로.
